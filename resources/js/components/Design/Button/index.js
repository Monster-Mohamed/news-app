...
const Button = ({words, url, className, click}) => {
    const splitWords = words.split(" ");
    const formatFirst = splitWords[0].split("");
    const formatSecond = splitWords[1].split("");
    const allFirstWord = formatFirst.map((el) => (<span>{el}</span>));
    const allSecondWord = formatSecond.map((el) => (<span>{el}</span>));
    return (<Link to={url} className={cl.button}>
  <span className={cl.button__text}>
      {allFirstWord}
  </span>
        <span> </span>
        {allSecondWord}
        <svg className={cl.button__svg} role="presentational" viewBox="0 0 600 600">
            <defs>
                <clipPath id={cl.myClip}>
